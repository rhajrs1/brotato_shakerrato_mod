class_name MultipleGroundAttackBehavior
...
export (int) var range_between_rings = 350
export (int) var number_rings = 3
...
var _current_cooldown = cooldown_per_rings
...
func shoot()->void :
	var distance = range_between_rings * _current_cycle
	var limitAngle = 2 * PI
	var angle = .0
	while angle < limitAngle:
		var pos = Vector2(_parent.global_position.x + cos(angle) * distance, _parent.global_position.y + sin(angle) * distance)
		var _projectile = spawn_projectile(0, pos)
		angle += projectile_angle
	
	if _current_cycle >= number_rings:
		_current_cooldown = cooldown
		_current_cycle = 1
	else:
		_current_cooldown = cooldown_per_rings
		_current_cycle += 1
